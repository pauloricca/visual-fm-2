...
- `MIDI Note On Send`: sends a MIDI note-on message to every connected MIDI output when its connection-only `trigger` input rises. `note`, `velocity`, and `channel` set the outgoing message.
- `MIDI Note Off Send`: sends a MIDI note-off message to every connected MIDI output when its connection-only `trigger` input rises. `note`, `velocity`, and `channel` set the outgoing message.
